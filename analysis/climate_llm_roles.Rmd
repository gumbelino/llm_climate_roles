...
dri_alpha_data <- read_csv("data/dri_alpha_data.csv", show_col_types = FALSE)

reg_data <- models %>% 
  select(provider:total_time_min) %>%
  left_join(dri_alpha_data, join_by(model)) %>%
  select(-arch) %>%
  filter(role != "all")

length(unique(reg_data$survey))

write_csv(reg_data, "data/roles_reg_data.csv")

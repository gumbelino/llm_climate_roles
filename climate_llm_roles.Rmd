---
title: 'Supplemental Materials. Deliberative Reasoning with(in) the Machine: Can Large Language Models Serve as System-0 Agents for Democratic Judgement?'
...
SURVEY_FILE <- "surveys_v5.xlsx"
LLMS_FILE <- "llms.csv"
CASES_FILE <- "deliberative_cases.csv"
...
write_csv(wilcox_res, "H1b_wilcox_results.csv")
...
ggplot(emm_df, aes(x = emmean, y = model)) +
  ...
  geom_vline(xintercept = grand_mean, linetype = "dashed", color = "red", linewidth = 0.8) +
...
```{r}
ggplot(role_model_heat, aes(x = model, y = role, fill = sd_rep)) +
  geom_tile() +
...
# ggplot(role_model_heat, aes(x = model, y = role)) +
#   geom_point(aes(size = sd_rep, color = sd_rep)) +
#   scale_color_distiller(palette = "RdYlGn", direction = -1) +
#   scale_size(range = c(3, 14)) +
#   theme_minimal()
# ggplot(role_model_heat, aes(model, sd_rep, fill = sd_rep)) +
#   geom_col() +
#   facet_wrap(~ role, ncol = 3) +
#   scale_fill_distiller(palette = "RdYlGn", direction = -1)
# ggplot(role_model_heat, aes(model, role, fill = sd_rep)) +
#   geom_tile() +
#   geom_contour(aes(z = sd_rep), color = "black", alpha = 0.3)
# 
# 
# ggplot(role_summary, aes(x = mean_dri, y = reorder(role, mean_dri))) +
#   geom_point(size = 3) +
#   geom_vline(xintercept = mean(df$dri), linetype = "dashed", color = "red") +
#   theme_minimal(base_size = 14) +
#   labs(
#     x = "Mean DRI",
#     y = "Role",
#     title = "Which Roles Produce Higher or Lower DRI?",
#     subtitle = "Red dashed line = overall DRI mean"
#   )
...
write_csv(sds, "detailed_analysis.csv")
...
create_heatmap <- function(data, cols, title, order = 1) {
  ...
  ggplot(plot_data, aes(x = column, y = row_label, fill = value)) +
    ...
    scale_fill_gradient2(low = "darkred", mid = "lightyellow", high = "darkgreen",
                         ...
                         na.value = "grey50") +
    labs(title = title, x = "Column", y = "Model/Role", fill = "Value") +
    theme_minimal() +
...
p1 <- create_heatmap(sds, c_cols, "Heatmap for C1:C33", order = 2)
print(p1)

p_cols <- paste0("P", 1:7)
p2 <- create_heatmap(sds, p_cols, "Heatmap for P1:P7", order = 2)
print(p2)

ggsave("plots/heatmap_C_columns.png", p1, width = 12, height = 8, dpi = 300)
ggsave("plots/heatmap_P_columns.png", p2, width = 8, height = 8, dpi = 300)
